...
object WechatGlobal {
        ...
        get() {
            if (!wxUnitTestMode) {
                if (!initChannel.isDone()) {
                    XposedBridge.log("ApkFile wait start")
                }
                initChannel.wait(INIT_TIMEOUT)
                initChannel.done()
                if (!initChannel.isDone()) {
                    XposedBridge.log("ApkFile wait end")
                }
            }
            return field
        }
        ...
        return if (wxUnitTestMode) {
            ...
            lazy(LazyThreadSafetyMode.PUBLICATION) {
                when (null) {
                    ...
                    wxProcessName -> throw Error("Invalid wxProcessName")
    ...
    fun init(lpparam: XC_LoadPackage.LoadPackageParam) {
        tryAsynchronously {
            ...
            try {
                XposedBridge.log("ApkFile start>>>>")
